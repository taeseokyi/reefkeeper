...
PLATEAU_SPAN_MPH = 2    # 최근 N개 max−min ≤ 2 milli-pH(=0.002) 면 평형. 정수 비교(float 지터 회피)
...
def main():
    ...
    print(f"평형 판정: 최근 {PLATEAU_N}개 span ≤ {PLATEAU_SPAN_MPH} mpH (정수 비교)")
            ...
            while True:
                ...
                if ph is None:
                    ...
                    if len(win) > PLATEAU_N:
                        win.pop(0)
                    span = (max(win) - min(win)) if len(win) >= PLATEAU_N else None
                    spanstr = f"{span}mpH" if span is not None else "-"
                    print(f"{elapsed:6d} {ph:7.3f} {d:+7.3f} {v:9.3f} {t:5.1f}  span{PLATEAU_N}:{spanstr}")
                    with open(CSV_FILE, 'a') as f:
                        f.write(f"{elapsed},{ph:.3f},{d:+.3f},{v:.3f},{t:.1f},"
                                f"{datetime.now():%H:%M:%S}\n")
                    prev = ph
                    if span is not None and span <= PLATEAU_SPAN_MPH:
                        print(f"\n[평형] 최근{PLATEAU_N} span={span}mpH≤{PLATEAU_SPAN_MPH} → pH 평탄(평형 도달) "
