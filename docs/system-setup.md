...
| 5 | `ron` | 폭기 | 에어 펌프 양쪽 동시 ON (참조수+수조수 폭기) |
| 6 | `wait:1800` | 폭기 | 동시 탈기 30분 (참조수+수조수 CO₂ 평형) |
| 7 | `airoff` | 폭기 | 에어 OFF |
| 8 | `ref` | 측정 | 참조수 pH 측정 |
| 9 | `m4b:10` | 측정 | 펌프4(b): 참조수 → 위즈 탱크 반환 |
| 10 | `m2f:10` | 측정 | 펌프2(f): 수조물 비이커 → pH 비이커 |
| 11 | `tank` | 측정 | 수조수 pH 측정 |
| 12 | `calkh` | 측정 | dKH 계산 + 이력 저장 |
| 13 | `m2b:10` | 정리 | 펌프2(b): 수조수 → 수조물 비이커 반환 |
| 14 | `m1b:30` | 정리 | 펌프1(b): 수조물 비이커 → 수조 반환 |
| 15 | `m3f:5` | 정리 | 펌프3(f): KCL 3% 저장수 공급 (프로브 보관) |

> **팁:** seq 명령은 시리얼 버퍼 크기(128바이트)의 제한이 있습니다. 명령이 길어지면 작업 절차를 나누어 순차적으로 실행할 수 있습니다. 예시:
> ```
> seq:settime:14|m3b:5|m1f:30|m4f:10|ron|wait:1800|airoff
